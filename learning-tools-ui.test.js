...
test("country comparison uses two to four valid countries and keeps years", () => {
  ...
  assert.equal(model.countries.length, 4);
  assert.equal(model.metrics.length, 6);
  model.metrics.forEach((metric) => {
    assert.equal(metric.observations.length, 4);
